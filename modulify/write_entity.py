...
def write_entity(working_directory, current_entity_name, imports, current_entity_result, current_entity_dependencies, file_name, app_name):
	...
	with open("__init.tmp", "a") as init_file:
    	init_file.write(f'from .{snake_case_name} import {current_entity_name}{os.linesep}')

    with open(working_directory + os.sep + snake_case_name + '.py', 'w') as new_entity_file:
